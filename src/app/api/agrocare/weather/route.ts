...
async function fetchWeatherData(lat: number, lon: number) {
  try {
    ...
    if (!apiKey) {
      ...
      mockData.locationName = "Mock Location (Nama dari Mock)";
      return mockData;
    }
...
function getMockWeatherData(lat: number, lon: number) {
  ...
  return {
    ...
    locationName: "Mock Location (Nama dari Mock)",
    current: {
      dt: currentDate,
...
async function generatePlantCareRecommendations(
  plantName: string,
  ...
  try {
    ...
    if (plantName === "Rice") {
      if (currentTemp > 30) {
        recommendation =
          "High temperature detected. Ensure adequate water supply. Consider increasing irrigation frequency.";
      } else if (currentTemp < 15) {
        recommendation =
          "Low temperature detected. Protect young seedlings. Delay fertilizer application until temperatures rise.";
      } else {
        recommendation =
          "Optimal temperature for rice growth. Maintain regular irrigation schedule and monitor for pests.";
      }

      if (humidity < 40) {
        recommendation +=
          " Low humidity may stress the plants. Consider light irrigation to increase ambient humidity.";
      }

      if (weatherCondition === "Rain") {
        recommendation +=
          " Rainy conditions may increase disease risk. Check for signs of fungal infections.";
      }
    } else if (plantName === "Coffee") {
      if (currentTemp > 28) {
        recommendation =
          "Temperature exceeding coffee's comfort zone. Ensure shade is adequate. Water in the early morning or evening.";
      } else if (currentTemp < 10) {
        recommendation =
          "Cold stress possible. Monitor plants closely. Avoid fertilization during cold periods.";
      } else {
        recommendation =
          "Good temperature range for coffee plants. Maintain regular care practices.";
      }

      if (humidity > 80) {
        recommendation +=
          " High humidity increases risk of fungal diseases. Ensure adequate spacing between plants for airflow.";
      }

      if (weatherCondition === "Clear") {
        recommendation +=
          " Sunny conditions may require additional irrigation, especially for younger plants.";
      }
    } else if (plantName === "Corn (Maize)") {
      if (currentTemp > 32) {
        recommendation =
          "High temperatures may affect pollination. Consider additional irrigation to reduce heat stress.";
      } else if (currentTemp < 10) {
        recommendation =
          "Cold stress can damage corn plants. Protect young seedlings if possible.";
      } else {
        recommendation =
          "Current temperature is acceptable for corn growth. Maintain regular care practices.";
      }

      if (humidity < 40) {
        recommendation +=
          " Low humidity may affect pollination. Consider irrigation during flowering.";
      }

      if (weatherCondition === "Rain") {
        recommendation +=
          " Monitor for water pooling which can cause root issues. Ensure fields have proper drainage.";
      }
    } else if (plantName === "Cassava") {
      if (currentTemp > 35) {
        recommendation =
          "High temperatures may slow growth. Ensure plants have adequate moisture.";
      } else if (currentTemp < 15) {
        recommendation =
          "Temperatures below optimal range. Growth may slow down. Reduce watering slightly.";
      } else {
        recommendation =
          "Good temperature range for cassava. Continue regular maintenance.";
      }

      if (humidity > 85) {
        recommendation +=
          " High humidity increases disease risk. Monitor for signs of root rot.";
      }

      if (weatherCondition === "Clear") {
        recommendation +=
          " Sunny conditions are favorable for cassava. Ensure adequate but not excessive irrigation.";
      }
    } else {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const recommendations = await generatePlantCareRecommendations(
      plantName,
      weatherData,
    );
